...
export default function AccountingSalesPage() {
  ...
  const [activeView, setActiveView] = useState<"customers" | "invoices" | "receipts">(
    "customers",
  );
  ...
  return (
    ...
    >
      {(customersError || invoicesError || receiptsError) ? (
        <Alert variant="destructive">
          <AlertTitle>Unable to load sales data</AlertTitle>
          <AlertDescription>
            {getApiErrorMessage(customersError || invoicesError || receiptsError)}
          </AlertDescription>
        </Alert>
      ) : null}
      ...
      <VerticalDataViews
        ...
        value={activeView}
        onValueChange={(value) => setActiveView(value as "customers" | "invoices" | "receipts")}
