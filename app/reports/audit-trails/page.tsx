...
export default function AuditTrailsReportPage() {
  ...
  const rows = useMemo<AuditRow[]>(() => {
    const correctionRows: AuditRow[] = (correctionsData?.data ?? []).map(
      (row) => ({
        id: `gold-${row.id}`,
        at: row.createdAt,
        module: "GOLD",
    ...
    const movementRows: AuditRow[] = (movementsData?.data ?? []).map((row) => ({
      ...
      actor:
        row.approvedBy ?? row.requestedBy ?? row.issuedBy?.name ?? "System",
      site: row.item.site.name,
      details: `${row.item.name} ${row.quantity} ${row.unit}`,
    }));

    const workOrderRows: AuditRow[] = (workOrdersData?.data ?? []).map(
      (row) => ({
        id: `maint-${row.id}`,
        at: row.createdAt,
        module: "MAINTENANCE",
  ...
  const pageError =
    sitesError || correctionsError || movementsError || workOrdersError;
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <NumericCell align="left">
            {format(new Date(row.original.at), "MMM d, yyyy HH:mm")}
          </NumericCell>
        ),
        size: 128,
        minSize: 128,
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Badge
            variant={
              row.original.module === "GOLD"
                ? "secondary"
                : row.original.module === "STORES"
                  ? "outline"
                  : "default"
            }
          >
      ...
      <section className="space-y-3">
        <header className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            {filteredRows.length} audit events
          </p>
        </header>
        {isLoading ? (
