...
export default function ScrapMaterialsPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["scrap-materials", queryState.search],
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) =>
      fetchJson(`/api/scrap-metal/materials/${id}`, { method: "DELETE" }),
    onSuccess: (result) => {
      toast({
  ...
  return (
    <ScrapShell
      title="Materials"
      description="Manage recyclable materials, default pricing anchors, and active catalog coverage."
      actions={
        <Button
          size="sm"
          onClick={() => {
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent size="lg">
          <DialogHeader>
            ...
            <DialogDescription>Keep the material catalog tight and operator-friendly.</DialogDescription>
          </DialogHeader>
          <form
            className="space-y-4"
            onSubmit={(event) => {
          ...
          >
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <Input
                ...
                onChange={(event) =>
                  setForm((prev) => ({ ...prev, defaultPricePerKg: event.target.value }))
                }
            ...
            <Select
              value={form.isActive}
              onValueChange={(value) => setForm((prev) => ({ ...prev, isActive: value as "true" | "false" }))}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
