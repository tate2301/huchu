          ...
          <div className="overflow-hidden rounded-md border-0 shadow-[var(--surface-frame-shadow)]">
            {runRows.map((run) => (
              ...
              >
                ...
                <div>
                  <Badge variant={options?.statusVariant?.(run) ?? "neutral"}>{run.status}</Badge>
                </div>
                <div>
  ...
  const periodColumns = useMemo<ColumnDef<PayrollPeriodRecord>[]>(
    () => [
        ...
        maxSize: 420},
      {
        ...
        header: "Status",
        cell: ({ row }) => (
          <Badge variant={row.original.status === "APPROVED" ? "success" : "warning"}>{row.original.status}</Badge>
        ),
        size: 120,
        minSize: 120,
        maxSize: 120},
  ...
  const archivedRunColumns = useMemo<ColumnDef<PayrollRunRecord>[]>(
    () => [
        ...
        maxSize: 420},
      {
        accessorKey: "status",
        header: "Status",
        cell: ({ row }) => <Badge variant={row.original.status === "POSTED" ? "success" : "warning"}>{row.original.status}</Badge>,
        size: 120,
        minSize: 120,
        maxSize: 120},
      {
