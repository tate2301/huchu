...
function mapSalarySettings(config: Awaited<ReturnType<typeof fetchPayrollConfig>>): SalarySettingsForm {
  return {
    payrollCycle: config.payrollCycle,
    autoGeneratePayrollPeriods: String(config.autoGeneratePayrollPeriods) as "true" | "false",
...
export default function SalaryPayrollPage() {
  ...
  const [settingsOverrides, setSettingsOverrides] = useState<Partial<SalarySettingsForm>>({})
  ...
  const { data: runsData, isLoading: runsLoading, error: runsError } = useQuery({
    ...
    queryFn: () => fetchPayrollRuns({ domain: "PAYROLL", periodId: activePeriodId, limit: 200 }),
  ...
  const saveSettingsMutation = useMutation({
    mutationFn: async (payload: SalarySettingsForm) =>
      updatePayrollConfig({
        ...
        autoGeneratePayrollPeriods: payload.autoGeneratePayrollPeriods === "true",
    ...
    >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Switch between salary payroll and gold payout payroll.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <CardDescription>Select one salary payroll period to work on.</CardDescription>
            </div>
        ...
        <CardContent>
          {periodsLoading ? (
            ...
            <div className="text-sm text-muted-foreground">No salary payroll periods available.</div>
          ) : (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Generate a salary run from approved compensation setup.</CardDescription>
        </CardHeader>
        <CardContent>
          {!activePeriod ? (
            ...
            >
              <div>
                <label htmlFor="salary-run-notes" className="mb-2 block text-sm font-semibold">Run Notes</label>
                <Input
                  id="salary-run-notes"
                  value={runNotes}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Review, submit, and approve salary runs in sequence.</CardDescription>
        </CardHeader>
        <CardContent>
          {!activePeriod ? (
      ...
      <Sheet open={settingsOpen} onOpenChange={setSettingsOpen}>
        <SheetContent className="w-full sm:max-w-lg p-6">
          <SheetHeader>
            ...
            <SheetDescription>Configure recurring salary periods and generation horizon.</SheetDescription>
          </SheetHeader>
          {configLoading || !settingsForm ? (
            <div className="mt-6"><Skeleton className="h-24 w-full" /></div>
          ) : (
            <form className="mt-6 space-y-4" onSubmit={(event) => { event.preventDefault(); saveSettingsMutation.mutate(settingsForm) }}>
              <div>
                <label className="mb-2 block text-sm font-semibold">Payroll Cycle</label>
                <Select value={settingsForm.payrollCycle} onValueChange={(value) => setSettingsOverrides((prev) => ({ ...prev, payrollCycle: value as SalarySettingsForm["payrollCycle"] }))}>
              ...
              <div>
                <label className="mb-2 block text-sm font-semibold">Auto Generate Payroll Periods</label>
                <Select value={settingsForm.autoGeneratePayrollPeriods} onValueChange={(value) => setSettingsOverrides((prev) => ({ ...prev, autoGeneratePayrollPeriods: value as SalarySettingsForm["autoGeneratePayrollPeriods"] }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                <label htmlFor="salary-horizon" className="mb-2 block text-sm font-semibold">Generation Horizon</label>
                <Input id="salary-horizon" type="number" min="1" max="12" value={settingsForm.periodGenerationHorizon} onChange={(event) => setSettingsOverrides((prev) => ({ ...prev, periodGenerationHorizon: event.target.value }))} />
              </div>
      ...
      <Sheet open={manualPeriodOpen} onOpenChange={setManualPeriodOpen}>
        <SheetContent className="w-full sm:max-w-lg p-6">
          <SheetHeader>
            ...
            <SheetDescription>Use manual periods for contractor or exception scenarios.</SheetDescription>
          </SheetHeader>
          <form className="mt-6 space-y-4" onSubmit={(event) => { event.preventDefault(); createManualPeriodMutation.mutate(manualPeriodForm) }}>
            <div>
              <label htmlFor="salary-month" className="mb-2 block text-sm font-semibold">Month</label>
              <Input id="salary-month" type="month" value={manualPeriodForm.monthKey} onChange={(event) => setManualPeriodForm((prev) => ({ ...prev, monthKey: event.target.value }))} />
            </div>
            <div>
            ...
            <div>
              <label htmlFor="salary-due" className="mb-2 block text-sm font-semibold">Due Date</label>
              <Input id="salary-due" type="date" value={manualPeriodForm.dueDate} onChange={(event) => setManualPeriodForm((prev) => ({ ...prev, dueDate: event.target.value }))} />
            </div>
            <div>
              <label htmlFor="salary-notes" className="mb-2 block text-sm font-semibold">Notes</label>
              <Input id="salary-notes" value={manualPeriodForm.notes} onChange={(event) => setManualPeriodForm((prev) => ({ ...prev, notes: event.target.value }))} />
            </div>
      ...
      <Dialog open={Boolean(runDetailsId)} onOpenChange={(open) => { if (!open) setRunDetailsId(null) }}>
        <DialogContent className="max-w-5xl">
          <DialogHeader>
            ...
            <DialogDescription>Detailed salary line amounts and rule components.</DialogDescription>
          </DialogHeader>
          {runDetailsLoading ? (
