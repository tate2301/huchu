...
export async function GET(request: Request) {
  ...
  try {
    const [admins, users, requests, sessions] = await Promise.all([
      services.admin.list({ companyId, search: search || undefined, limit: 100 }),
      services.user.list({ companyId, search: search || undefined, limit: 100 }),
