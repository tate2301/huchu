...
import { canAccessCapabilityWithToken, canAccessRouteWithToken } from "./gating/enforcer";
import {
...
import { getAdminRootDomain, isAdminPortalHost, isSuperuserRole } from "./admin-portal";
import { buildCallbackLoginPath } from "./auth-core/redirects";
import { isAuthExpired } from "./auth-core/session-policy";
import {
  isRoleRouteRestricted,
  isRouteAllowedForRole,
  landingPathForRole,
} from "./auth-core/role-routes";
import { registeredPortal, registeredPortals, registeredRoleRestrictedRoutes, type PortalEntry } from "./manifest";
import { PUBLIC_BASE_PATHS } from "./public-routes";
...
/** The portal trees on this host: the registered modules' portals, and the admin portal the kernel serves. */
...
function registeredPortalHomeForRole(role: string | undefined | null): PortalEntry | null {
  if (!role) {
    return null;
  }
...
function getPortalHomeForRole(role: string | undefined | null) {
  const portal = registeredPortalHomeForRole(role);
...
export function createProxy() {
  ...
  async function proxy(request) {
    ...
    // A portal that pins its roles to its host (the till): a cashier who lands
    // on the tenant host is sent to the portal host, whatever they asked for.
    const pinnedPortal = !isApiRequest && token ? registeredPortalHomeForRole(token.role) : null;
    ...
    if (hostContext.portalPath && !isApiRequest) {
      ...
      // A portal with public paths (the till) serves them bare on its host and
    ...
    // Routes only some roles may reach, as the modules declare them.
    if (token) {
