...
import { snapshotGoldUsdValue } from "../../../gold/valuation";
import { recordInventoryEvent } from "../../../gold/inventory";
import { assertPeriodOpen, PeriodClosedError } from "../../../gold/period-close";
  ...
  try {
    ...
    const pour = await prisma.$transaction(async (tx) => {
      ...
      await captureAccountingEvent({
        ...
        amount: created.valueUsd != null ? Number(created.valueUsd) : Number(created.grossWeight),
        payload: {
          siteId: created.siteId,
          grossWeight: Number(created.grossWeight),
          valueUsd: created.valueUsd != null ? Number(created.valueUsd) : null,
          goldPriceUsdPerGram: created.goldPriceUsdPerGram != null ? Number(created.goldPriceUsdPerGram) : null,
