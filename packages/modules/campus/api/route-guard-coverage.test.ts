...
const SCHOOLS_API = join(__dirname, "v2/schools");

/**
 * S-4.2 — the module-neutral record routes (`/api/v2/records/**`) need the same
 * coverage more than most: they are deliberately absent from the route registry,
 * so the feature gate that protects every other /api/v2 path does not run for
 * them. They are the records module's, so their copy of this test lives with
 * them (`records-route-guard-coverage.test.ts`), not here.
 */
