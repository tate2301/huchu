...
import { ArrowRight, FileText } from "@corelithzw/ui/lib/icons";
...
import { fetchEmployeePayments, type EmployeePayment } from "../../../../api-client";
...
export default function OutstandingSalariesPage() {
    ...
    () =>
      payments
        .map((payment) => {
          ...
          const overdueDays =
            outstandingAmount > 0
              ? Math.max(differenceInCalendarDays(today, dueDate), 0)
              : 0;
          return {
            ...payment,
            outstandingAmount,
  ...
  const filteredRows = useMemo(() => {
    ...
    return outstandingRows
      .filter((row) =>
        employeeFilter === "ALL" ? true : row.employeeId === employeeFilter,
      )
      .filter((row) =>
  ...
  const totals = useMemo(() => {
    ...
    const employeesAffected = new Set(filteredRows.map((row) => row.employeeId))
      .size;
  ...
  const summaryUnit = useMemo(() => {
    const uniqueUnits = Array.from(
      new Set(filteredRows.map((row) => row.unit)),
    );
  ...
  const columns = useMemo<ColumnDef<RowView>[]>(
    () => [
          ...
          <div>
            ...
            {row.original.isOverdue ? (
              <div className="text-xs text-red-600">
                {row.original.overdueDays}d overdue
              </div>
            ) : null}
          </div>
        ),
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Badge
            variant={row.original.status === "PARTIAL" ? "warning" : "neutral"}
          >
            {row.original.status}
          </Badge>
        ),
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <NumericCell>
            {toCurrency(row.original.amount, row.original.unit)}
          </NumericCell>
        ),
        size: 120,
        minSize: 120,
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <NumericCell>
            {toCurrency(row.original.paidAmount ?? 0, row.original.unit)}
          </NumericCell>
        ),
        size: 120,
        minSize: 120,
      ...
      <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
        <Card>
          <CardHeader>
            ...
            <CardTitle>
              {toCurrency(totals.totalOutstanding, summaryUnit)}
            </CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            ...
            <CardTitle>
              {toCurrency(totals.overdueOutstanding, summaryUnit)}
            </CardTitle>
          </CardHeader>
        </Card>
        <Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle>
              {toCurrency(totals.bucket60 + totals.bucket90, summaryUnit)}
            </CardTitle>
          </CardHeader>
        </Card>
      </div>
      ...
      >
        <DialogContent size="lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Audit the source chain and outstanding balance for this line.
            </DialogDescription>
          </DialogHeader>
          {selected ? (
            <div className="grid gap-3 text-sm">
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <div>
                  ...
                  <div className="font-medium font-mono">
                    {toCurrency(selected.amount, selected.unit)}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Paid</div>
                  <div className="font-medium font-mono">
                    {toCurrency(selected.paidAmount ?? 0, selected.unit)}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Outstanding</div>
                  <div className="font-medium font-mono">
                    {toCurrency(selected.outstandingAmount, selected.unit)}
                  </div>
                </div>
              </div>
              <div className="grid gap-3 md:grid-cols-3">
                <div>
                  ...
                  <div className="font-mono">
                    {format(new Date(selected.periodStart), "yyyy-MM-dd")}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Period End</div>
                  <div className="font-mono">
                    {format(new Date(selected.periodEnd), "yyyy-MM-dd")}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Due Date</div>
                  <div className="font-mono">
                    {format(new Date(selected.dueDate), "yyyy-MM-dd")}
                  </div>
                </div>
              </div>
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div>
                  ...
                  <div>
                    {selected.disbursementBatch
                      ? selected.disbursementBatch.code
                      : "-"}
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap justify-end gap-2 pt-2">
                {selected.payrollRun ? (
                  <Button asChild size="sm" variant="outline">
                    <Link
                      href={`/payroll/runs?runId=${selected.payrollRun.id}`}
                    >
                      Open Run
                    </Link>
                  </Button>
                ...
                {selected.disbursementBatch ? (
                  <Button asChild size="sm" variant="outline">
                    <Link
                      href={`/payroll/disbursements?batchId=${selected.disbursementBatch.id}`}
                    >
                      Open Batch
                    </Link>
                  </Button>
                ) : null}
                <Button onClick={() => setDetailsId(null)} size="sm">
                  Close
                </Button>
              </div>
            </div>
