...
import { fetchCompensationProfiles, type CompensationProfileRecord, fetchCompensationRules, type CompensationRuleRecord, fetchCompensationTemplates, type CompensationTemplateRecord, fetchDepartments, fetchEmployees, fetchJobGrades } from "../../../api-client";
...
export default function CompensationPage() {
  ...
  const templateRuleOptions = useMemo(
    () =>
      rules.filter(
        (rule) => rule.workflowStatus === "APPROVED" && rule.isActive,
      ),
    [rules],
  );
  ...
  const openRejectionDialog = useCallback(
    (target: NonNullable<RejectionTarget>) => {
  ...
  const templateColumns = useMemo<ColumnDef<CompensationTemplateRecord>[]>(
    () => [
      ...
      },
      {
        ...
        header: "Base",
        cell: ({ row }) =>
          `${row.original.currency} ${row.original.baseAmount.toFixed(2)}`,
        size: 160,
        minSize: 160,
        maxSize: 160,
      },
      {
        ...
        header: "Rules",
        cell: ({ row }) =>
          row.original._count?.rules ?? row.original.rules.length,
        size: 160,
        minSize: 160,
        maxSize: 160,
      },
        ...
        cell: ({ row }) => (
          <div className="text-right">
            <Button asChild size="sm" variant="outline">
              <Link href={`/people?templateId=${row.original.id}`}>
                Use in Onboarding
    ...
    () => [
        ...
        cell: ({ row }) =>
          `${row.original.effectiveFrom.slice(0, 10)} to ${
            row.original.effectiveTo
              ? row.original.effectiveTo.slice(0, 10)
              : "Open"
          }`,
        size: 128,
        minSize: 128,
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Badge
            variant={
              row.original.workflowStatus === "APPROVED" ? "success" : "neutral"
            }
          >
              ...
              {status === "SUBMITTED" ? (
                <>
                  <Button
                    ...
                    onClick={() =>
                      approveProfileMutation.mutate(row.original.id)
                    }
                  >
                    Approve
                  </Button>
      ...
      <Sheet open={templateOpen} onOpenChange={setTemplateOpen}>
        <SheetContent size="lg" className="w-full p-6">
          <SheetHeader>
            ...
            <SheetDescription>
              Templates combine a base profile and approved rules for faster
              onboarding.
            </SheetDescription>
          </SheetHeader>
          <form
            ...
            <div>
              ...
              {templateRuleOptions.length === 0 ? (
                <div className="rounded-md border border-dashed p-3 text-xs text-muted-foreground">
                  No approved active rules available yet. Create and approve
                  rules first.
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-xs text-muted-foreground">
                          ({rule.type},{" "}
                          {rule.calcMethod === "PERCENT"
                            ? `${rule.value}%`
                            : `${rule.currency} ${rule.value}`}
                          )
                        </span>
                      </label>
                    );
      ...
      <Sheet open={profileOpen} onOpenChange={setProfileOpen}>
        <SheetContent size="md" className="w-full p-6">
          <SheetHeader>
            ...
            <SheetDescription>
              Define the base salary profile for an employee.
            </SheetDescription>
          </SheetHeader>
          <form
            className="mt-6 space-y-4"
          ...
          >
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setProfileForm((prev) => ({ ...prev, employeeId: value }))
                }
              >
                <SelectTrigger className="w-full">
      ...
      <Sheet open={ruleOpen} onOpenChange={setRuleOpen}>
        <SheetContent size="md" className="w-full p-6">
          <SheetHeader>
            ...
            <SheetDescription>
              Configure allowance or deduction logic.
            </SheetDescription>
          </SheetHeader>
          <form
            className="mt-6 space-y-4"
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setRuleForm((prev) => ({
                      ...prev,
                      calcMethod: value as RuleForm["calcMethod"],
                    }))
                  }
                >
                  <SelectTrigger className="w-full">
          ...
          <>
            <header className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Reusable profile + rule bundles for onboarding permanent,
                contract, or role-specific staff.
              </p>
            </header>
            {templatesLoading ? (
              ...
              <DataTable
                ...
                onQueryStateChange={(next) =>
                  setTemplatesQuery((prev) => ({ ...prev, ...next }))
                }
                features={{
                  sorting: true,
                  globalFilter: true,
        ...
        {activeView === "profiles" ? (
          <>
            <header className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Employee base pay with effective-date history.
              </p>
            </header>
            {profilesLoading ? (
              <Skeleton className="h-20 w-full" />
              ...
              <DataTable
                ...
                onQueryStateChange={(next) =>
                  setProfilesQuery((prev) => ({ ...prev, ...next }))
                }
                features={{
                  sorting: true,
                  globalFilter: true,
        ...
        {activeView === "rules" ? (
          <>
            <header className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Reusable allowance and deduction rules.
              </p>
            </header>
            {rulesLoading ? (
              <Skeleton className="h-20 w-full" />
