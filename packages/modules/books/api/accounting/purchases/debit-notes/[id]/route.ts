...
import { createJournalEntryFromSource } from "../../../../../posting";
import { recalcPurchaseBillBalance } from "../../../../../balances";
        ...
        try {
          ...
          if (postedEntry) {
            await createJournalEntryFromSource({
              ...
              entryDate: new Date(),
              description: `Void debit note ${updated.noteNumber}`,
              createdById: session.user.id,
              amount: updated.total,
              netAmount: updated.subTotal,
              taxAmount: updated.taxTotal,
          ...
          console.error("[Accounting] Purchase debit note void reversal failed:", error);
        }
      }
