...
vi.mock("./fdms-connector", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./fdms-connector")>();
  return { ...actual, issueWithFdmsConnector: issueMock, syncWithFdmsConnector: syncMock };
});

import type { FiscalSigningInput } from "./fiscalisation";
...
const {
  ...
  issueFiscalReceipt,
  sourceWhere,
} = await import("./fiscalisation");
const { openFiscalDay, closeFiscalDay } = await import("./fiscal-day");
const {
  centsFromMinorUnits,
  verifyReceiptChain,
  verifyReceiptSignature,
} = await import("./fdms-receipt-signing");
