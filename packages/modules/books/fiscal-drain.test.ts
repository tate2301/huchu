...
import { NotificationEntityType, NotificationSeverity, NotificationType } from "@corelithzw/db";
import { createNotification, getManagerIds } from "@corelithzw/module-notifications/service";
import {
  drainDueFiscalReceipts,
  fiscalRetryDelayMs,
  onFiscalBacklog,
  type FiscalBacklogEvent,
  type FiscalDrainIssuers,
} from "./fiscal-drain";

/**
 * What a host does with a backlog: raise an incident for the managers. The
 * drain only describes the backlog and fires this; the compliance module
 * owns the real emitter, so the test stands in for it with the same effect.
 */
async function raiseBacklogIncident(event: FiscalBacklogEvent) {
  await createNotification(prisma, {
    ...
    category: "OPS",
    recipientIds: await getManagerIds(event.companyId),
    entityType: NotificationEntityType.INCIDENT,
    entityId: event.incidentId,
  });
}
...
beforeAll(async () => {
  ...
  onFiscalBacklog(raiseBacklogIncident);
