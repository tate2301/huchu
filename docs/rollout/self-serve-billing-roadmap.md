...
| ID | Story | Acceptance signal | Status |
|---|---|---|---|
| SS-4.1 | As a founder, the gateway choice is made on evidence | Written evaluation memo — Paynow vs Pesepay vs ContiPay — against the fixed criteria: recurring-billing support and settlement time first, headline rate last (master-plan risk #10). The memo is the artifact; the decision lands in this changelog. **Decided 2026-08-20: Paynow**, recorded in `docs/rollout/environment-credentials.md` with the reasoning and what was deliberately not bought | `done` |
...
|---|---|---|---|
| 2026-08-20 | — | SS-4.1 → `done` | **Paynow.** Chosen on rail coverage, which is the criterion that survived contact with the market: the buyer and the payer are frequently different people here — a finance officer paying Grow by Visa and a shop owner paying Fiscal by EcoCash are the same product — and Paynow is the only one of the three reaching cards and mobile money in a single integration. Worth recording what was *not* bought: Pesepay advertises recurring payments, which reads like the scarcest capability in this market, but Zimbabwe has no true recurring-debit rail and mobile money is push-only, so it is a scheduled prompt rather than a charge. Choosing a provider for that would have been paying for a word. Annual prepay stays the answer to collections — twelve collections become one — which is what SS-4.3 exists to present first. The two unchosen adapters stay in the codebase so reversing this costs one line plus a key. `PAYMENT_PROVIDER="paynow"`; what remains is the merchant account, not the decision, and SS-4.2 stays `wip` until a sandbox payment moves a tenant `TRIALING` → `ACTIVE`. |
