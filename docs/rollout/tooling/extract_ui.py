...
# ---- 1. moves (git mv keeps history) --------------------------------------------
MOVES = [  # (old path relative to app, new path relative to package)
    ("components/ui", "components"),
    ("components/charts", "charts"),
    ("components/corelith", "corelith"),
    ("components/shared/employee-avatar.tsx", "components/employee-avatar.tsx"),
    ("lib/utils.ts", "lib/utils.ts"),
    ("lib/icons.tsx", "lib/icons.tsx"),
    ("lib/ui", "lib/ui"),
    ("lib/animation", "lib/animation"),
    ("lib/charts", "lib/charts"),
    ("lib/data-table-adapter.ts", "lib/data-table-adapter.ts"),
    ("hooks/use-debounced.ts", "hooks/use-debounced.ts"),
    ("hooks/use-mobile.tsx", "hooks/use-mobile.tsx"),
]
os.makedirs(f"{PKG}/lib", exist_ok=True); os.makedirs(f"{PKG}/hooks", exist_ok=True); os.makedirs(f"{PKG}/components", exist_ok=True)
for old, new in MOVES:
    src, dst = f"{APP}/{old}", f"{PKG}/{new}"
    assert os.path.exists(src), src
    if os.path.isdir(src) and os.path.isdir(dst):
        # merge directory contents (components/ui -> components which already holds employee-avatar)
        for entry in sorted(os.listdir(src)):
            sh(f'git mv "{src}/{entry}" "{dst}/{entry}"')
        os.rmdir(src) if os.path.isdir(src) and not os.listdir(src) else None
    else:
        os.makedirs(os.path.dirname(dst), exist_ok=True)
        sh(f'git mv "{src}" "{dst}"')

# ---- 2. import rewrite ------------------------------------------------------------
# old alias prefix -> new path inside the package (both without extension)
PREFIX_MAP = [
    ("components/shared/employee-avatar", "components/employee-avatar"),
    ("components/ui", "components"),
    ("components/charts", "charts"),
    ("components/corelith", "corelith"),
    ("lib/utils", "lib/utils"),
    ...
    ("lib/animation", "lib/animation"),
    ("lib/charts", "lib/charts"),
    ("lib/data-table-adapter", "lib/data-table-adapter"),
    ("hooks/use-debounced", "hooks/use-debounced"),
    ("hooks/use-mobile", "hooks/use-mobile"),
]
alts = "|".join(re.escape(p) for p, _ in PREFIX_MAP)
SPEC = re.compile(r'(["\'])@/(' + alts + r')(/[^"\']*)?\1')

def resolve_target(prefix, rest):
...
def rewrite_file(path, in_package):
    ...
    def repl(m):
        q, prefix, rest = m.group(1), m.group(2), m.group(3)
        target = resolve_target(prefix, rest)
        if in_package:
            rel = os.path.relpath(f"{PKG}/{target}", os.path.dirname(path))
...
changed_app = sum(rewrite_file(p, False) for p in walk(APP))
changed_pkg = sum(rewrite_file(p, True) for p in walk(PKG))
print(f"rewritten: app files={changed_app} package files={changed_pkg}")
...
''', 1),
    ("DocumentExportFormat", "TableExportFormat", 3),
