...
MODS="platform shell modules/records modules/documents modules/notifications modules/books modules/people modules/stock modules/maintenance modules/compliance modules/gold modules/sell modules/crm modules/campus modules/offline"
...
echo "== boundary tests"; for pkg in $MODS; do [ "$pkg" = platform ] || [ "$pkg" = shell ] && continue; (cd /home/user/huchu/packages/$pkg && pnpm exec vitest run module-boundary.test.ts 2>&1 | grep -E "Test Files|FAIL|×|Error" | head -6); done
echo "== package suites"; for pkg in $MODS; do [ "$pkg" = platform ] || [ "$pkg" = shell ] && continue; echo "-- $pkg"; (cd /home/user/huchu/packages/$pkg && pnpm exec vitest run 2>&1 | grep -E "Test Files|Tests |FAIL|×" | head -12); done
