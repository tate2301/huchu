...
| ST-1.2 | As a tenant admin on a parked accounting feature, the page is gone but my data is not | Banking-reconciliation, financial-statements, cost-center and currency pages removed from `lib/accounting/tab-config.ts` navigation and from every sellable bundle; models untouched; executive dashboard cash tiles still render | `done` |
...
| ST-3.2 | As an engineer, `FixedAsset` and `Budget`/`BudgetLine` are gone; `JournalLine.costCenterId` and the `CostCenter` model remain | Migration applies clean on a copy of production data; posting-engine tests green | `done` |
| ST-3.3 | As an engineer, dead `AccountingSourceType` values are pruned | `SCRAP_METAL_PURCHASE`, `SCRAP_METAL_BATCH`, `SCRAP_METAL_SALE` and other breadcrumb-only values with no seeded posting rule removed from the enum and from `lib/accounting/source-types.ts`; existing `AccountingIntegrationEvent` rows for removed types handled by the migration (retained with a string column or archived per ST-0.2) | `done` |
| ST-3.4 | As a buyer, `ADDON_ACCOUNTING_ADVANCED` sells what still exists | Bundle contents pruned to the surviving features (AR/AP, banking, cost centers, multi-currency); `accounting.fixed-assets` and `accounting.budgets` removed from `FEATURE_CATALOG`; `docs/platform-pricing-feature-flags-and-modules.md` updated in the same PR | `done` |
...
|---|---|---|---|
| 2026-08-19 | — | ST-1.2, ST-4.1 → `done` | **The parked accounting surfaces left the navigation, and the freeze register became a rule somebody can be pointed at.** Banking reconciliation, currency rates, cost centers and financial statements are gone from `ACCOUNTING_TABS`, from the accounting hub's link cards, from the reporting hub, and from the sidebar's accounting section. Their pages, APIs, models and feature keys are untouched. **One deviation from this story's own acceptance signal, deliberately.** It asked for the four to leave *every sellable bundle* as well, and doing that would have broken the same row's other requirement that the executive dashboard's cash tiles keep rendering: `accounting.banking` is what gates those tiles and the cash trend in `app/api/dashboard/executive-overview/route.ts`, and `accounting.financial-statements` gates the general-ledger and cash-flow report APIs plus the reporting hub, none of which the trim was ever meant to reach. So parking here is navigation, not entitlement, and the question of whether a bundle should still charge for a feature nobody can navigate to is handed to PR-4 as a pricing decision rather than settled quietly in a nav commit. Note the shape of it: `ADDON_ACCOUNTING_ADVANCED` now lists five features of which three have no entry point. ST-4.1: CONTRIBUTING gained a scope section distinguishing dropped (code and schema gone — do not restore a feature key to make an old test pass; fix the test), parked (UI out of nav, everything underneath intact), and frozen (Maintenance — still shipping, still release-blocking, no new stories, because freezing a module retires its roadmap and not its users). Its stale CCTV subsection went with the module. |
| 2026-08-19 | `1913f03` | ST-3.1 – ST-3.4 → `done` | The tables are gone. Ordered dependent-side first so that every SalesInvoice, PurchaseBill, Vendor, PurchasePayment, SalesReceipt and Customer a scrap row pointed at survived the drop; the export in `fbc6100` holds the association so it stays recoverable. StreamSession and PlaybackRecord went with CCTV — both carry a required FK to Camera. CostCenter and its columns stayed, because the posting engine still writes them. **Correction to the row below:** `a08925d` recorded ST-3 as deliberately deferred pending a real export against real data. That was true when it was written; the work landed immediately afterwards and this row supersedes it. The caution still stands for whoever runs this against production — the migration is safe, but the export should be run and checked first. |
| 2026-08-19 | _this change_ | ST-3.1 – ST-3.4 → `done` | The schema went. One migration, `20260819090000_scope_trim_drop_dropped_module_schema`: six CCTV tables (including `StreamSession` and `PlaybackRecord`, which hold a required FK to `Camera` and could not outlive it), four `CarSales*`, ten `Scrap*`, plus `FixedAsset`, `Budget` and `BudgetLine` — and their fifteen enum types. `CostCenter` and every `costCenterId` column stay: the posting engine allocates to them. Scrap was dropped from the dependent side, proven on a fixture holding one scrap purchase and one scrap sale wired to a real bill, payment, vendor, invoice, receipt and customer — all six survived at unchanged row counts. The three `SCRAP_METAL_*` `AccountingSourceType` values are pruned; the `AccountingIntegrationEvent` rows naming them keep every field and lose only the typed pointer (the column is nullable, so `sourceDomain`, `sourceAction`, `description` and `payloadJson` still say what happened), `PostingRule` rows for those types are deleted as configuration that can never fire, and the migration **aborts** rather than rewrite a `JournalEntry` or `PaymentLedgerEntry` holding one. `IRREGULAR_PAYOUT_DISBURSEMENT` was left in place for the reason `lib/workflow/approvals.test.ts` records: it was captured PENDING and could have reached a journal. `WorkspaceProfile.SCRAP_METAL` and `.AUTOS` are **retained** — the column is NOT NULL, dropping them would fail the migration on any tenant still stored on one, and `toActiveWorkspaceProfile` already degrades them to `GENERAL`. |
| 2026-08-18 | `fbc6100`, `a08925d` | ST-0.2, ST-2.1 – ST-2.5 → `done` | The export exists (per company, every row of the CCTV/autos/scrap tables, plus the surviving accounting documents scrap rows pointed at — and StreamSession/PlaybackRecord, which this roadmap missed: both carry a required FK to Camera and could not have survived its drop). Then the code went: pages, APIs, components, libs, the CCTV gateway server, the dead fixed-asset and budget registers, the thrift stub, and the retired workspace-profile handling. `lib/commodity-billing.ts` survived, as the standing instruction requires. **ST-3 remains `todo` on purpose** — the tables still hold their data, the export has only been proven against an empty local database, and dropping ScrapMetal* severs FKs into SalesInvoice, PurchaseBill and Vendor. That step wants a real export against real data first. — _superseded 2026-08-19: ST-3 shipped, and the export was rewritten to read the dropped tables through raw SQL with a `to_regclass` guard so it still runs against a restored pre-migration database._ |
