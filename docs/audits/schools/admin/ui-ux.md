...
The setup screens work but are hard to reach and hard to scan. The six master-data pages sit behind two navigation rails and a description line, then repeat their counts in chips and tabs, stack their filters vertically with the create button floating between them, and add a third control row for the table's own search. The timetable, the one screen that should be a grid, never shows one: three header buttons with the primary greyed, filters over two rows, and an empty state that explains what a lesson is. Teachers and guardians lists clip their actions column off the right edge. Title Case and ALL CAPS labels appear on documents, notices, boarding and years. Native date inputs render in the browser's locale on every date field. The imports wizard has the best copy in the module and then grows down the page instead of advancing. The identity settings page is the one place that gates its controls by role and explains it; nothing else copied it. Since this was written the six master-data pages have become reachable from the school rail through a Setup band, though they still open in the Management shell with its own rail, its Title Case title and its description line; the teachers list has lost its repeated "Find the employee" button, its Title Case and its second name, and both it and the guardians list put their row verbs in a menu; documents, notices, years, allocations and leave name themselves once in sentence case and the notices rationale card is gone; library loans has an app-bar title of its own; and `app/schools` has route-level loading, error and not-found states with skeletons that mirror the real layouts. The stacked filters, the duplicated chips, the timetable with no grid, the native date inputs and the missing phone renderers are all as described below.

## Runtime check (14 September 2026)

Verified on the seeded St Marys tenant as the head (see `../reference/runtime-verification.md`). Confirmed at runtime: the master-data pages are absent from the school sidebar; the classes, timetable and teachers screens render as described, with "Find the employee" on every teacher row. Not reproduced: the clipped Actions column on the teachers table.
...
| `/schools/calendar` | `school-calendar-page-content.tsx` → `school-days-content.tsx` | `PageChrome` (good); two `CreateButton`s; native date inputs | Edit is delete and recreate; the API has since gained a `PATCH` that this screen does not call | gated | present | P2 |
...
| `/schools/teachers` | `schools-teachers-content.tsx` (1,027 lines) | `PageHeading` plus caption duplicating the band; "Add Subject" Title Case; Title Case tabs; three control rows; "Find the employee" repeated on all eight rows (confirmed at runtime); the clipped Actions column seen in the older screenshot set was not reproduced at 1440px. Fixed: `PageChrome` names the screen once as "Teaching staff", the Title Case is gone, the HR badge is itself the link to the record, "Find the employee" is a row-menu verb shown only where there is no employee to find, and the row verbs are a menu. The band chips, the tabs, the three control rows and the profile-flags column stand | Three tabs, three deletes — the deletes now read as archiving with a way back; subjects CRUD here and in master data | gated | renderer ×3 (good) | P2 (was P1) |
...
| `/schools/documents` | `school-documents-content.tsx` (938 lines) | "School Documents", "Report Cards / Fee Invoices / Class Lists / Attendance Registers", "Select Student", "Student No", "Invoice No" all Title Case; raw enum status printed. Fixed: `PageChrome` names it "Documents" once, the tabs and labels are sentence case, and the rationale and stat cards above them are gone | Pupil → preview → print is three clicks; batch by class only for lists and registers; four of eight sources missing; and a report card now fails outright at a school with no open publish window, which is every school (`workflows.md` B14) | not gated (fine) | no renderer; fixed-width preview | P1 |
...
| `/schools/library`, `/library/loans` | `library-content.tsx`, `library-loans-content.tsx` | `PageChrome` (good); both register the app-bar title "Library", fixed: the loans screen registers "Library loans"; two primaries each | Lending is three pickers; no barcode or accession quick entry; "Fines if back today" chip is good | gated | no renderer | P1 |
...
| `/schools/notices` | `schools-notices-content.tsx` | "School Notices" Title Case; side card "A notice cannot be recalled" (`:576-579`) rendered as UI; `toLocaleString` ×6. Fixed: the rationale card is gone and `PageChrome` names it "Notices" | No schedule, draft or recall | gated | none | P2 (was P0; the copy leak is gone) |
| `/schools/messages` | `office-inbox-content.tsx` | `PageHeading` plus description; assign-dialog description is rationale | **This audit was wrong to record a reply in the page.** There was none. The screen's own header comment put starting and replying with the family and the teacher in their own portals, and the route carried no reply action for anybody, so the office could route a fee question to a teacher who cannot read the ledger, or end the conversation, and nothing else. A reply composer now sits in the reading dialog, gated on `schools.reports` reply and hidden rather than refused for a reader without the grant and on an ended conversation (`workflows.md` B15). "End the conversation" confirm is good; still no unread badge in nav | gated | none: an inbox with no phone shell | P1 |

## 3. Screenshot findings

- **Subjects list (records-desktop).** Four levels of navigation (school sidebar, Settings rail, Master Data rail, page tabs) before the table, a description line, chips, stacked filters, a floating primary and a third control row. Mono codes as links and Core/Elective badges are good. The first of those four levels has gone: the school rail reaches this page directly from a Setup band. The Settings and Master Data rails, the description line, the chips, the stacked filters and the third control row are all still there.
- **Classes.** Two navigation rails plus the sidebar; "The year-group ladder and the streams inside each one." as a description; chips and tabs duplicated; "New class" floating at the right of the second filter; a third control row; "Capacity -"; pagination for six rows; a 740px table in a 1440px window. Reachable from the school rail now; nothing else in this paragraph has changed.
- **Teachers and teacher-HR.** Identity cell with mono id and email (good). Heading, caption and bar title; "Add Subject" in Title Case; three control rows; "Find the employee" eight times. The runtime capture shows the primary in the bar and no clipped column, so the older screenshot predates a fix. The repeated button has since gone with it: the HR badge is the link, and "Find the employee" is a row-menu verb on a teacher who has no employee. The heading and caption are one `PageChrome` title and the Title Case is swept; the three control rows stand.
- **Timetable (desktop and phone).** "No periods yet" banner with a real next step (good). Two-line description; three section-header buttons with the primary greyed; filters split over two rows with the search orphaned; nothing to click to place a lesson; on the phone, description, three buttons, a 190px banner with a wrapped button, then five stacked selects; no grid. Unchanged, except that the route now has a skeleton drawing the week grid with the days as headers rather than a list of rows.
...
- **Teachers on a phone.** Cards read well ("TCH-001 · Mathematics · No HR record · HOD · Class teacher"), but heading, caption, full-width primary, chips, band buttons, clipped tabs, two selects, search, Search, Columns/Export precede the first card at y≈610. The heading and caption are one title now; the rest of that stack is as it was.

## 4. Rule violations that recur

| Rule | Violation | Where |
|---|---|---|
| A page is named once | `MasterDataShell` or `PageHeading` plus the bar title | all six master-data pages, timetable, imports, messages; fixed on teachers, documents and notices |
...
| Row verbs in a menu | Clipped actions column; identical per-row button | teachers; fixed, the verbs are a menu and the repeated button is gone |
| Sentence case | Title Case and ALL CAPS | the six master-data page titles; fixed on documents, notices, teachers, years and boarding |
| Dates as `3 June 2026` | Native date inputs | calendar, timetable, imports, documents |
| Strip explanatory copy | Descriptions under every title; paragraphs under controls; prose empty states | most setup screens; fixed on documents, notices and the oversight boards, where the rationale cards and the stat cards repeating band chips are gone |
| One table per view | Three tables on the hostel page | hostels |
| Distinct app-bar titles | "Library" twice | library, loans; fixed, the loans bar reads "Library loans" |
...
**P0**
1. Remove the rationale card from notices (`schools-notices-content.tsx:576-582`). Done, and with it the rationale cards and the stat cards repeating band chips on the other back-office screens that drew them.
2. Bring the Setup band into the school sidebar (see workflows B1) so these screens are reachable in one rail. Done. The band links across to the pages where they are; they still open in the Management shell, so item 3 below stands in full.

**P1**
3. Replace `MasterDataShell` descriptions and `PageHeading`s with `PageChrome` (pass `description={undefined}` in `app/management/master-data/schools/*/page.tsx` as the minimum); move the create verb into the bar; delete captions that repeat chips. Done on teachers, documents and notices; not done on the six master-data pages, which still carry a Title Case shell title and a description.
...
5. Teachers: fold HR RECORD into a badge that is the link; move "Find the employee" into the row menu or a band action "Link 8 teachers to HR"; `layout="menu"` on the actions; hide PROFILE FLAGS by default. Done but for the last: the profile-flags column is still shown, in sentence case.
...
7. Title Case sweep: `school-documents-content.tsx:687, 770-773, 782, 267-272, 406-411`; `schools-notices-content.tsx:369`; `boarding-allocations-content.tsx:258, 294-298, 338, 388`; `boarding-leave-content.tsx:214-215, 223`; `schools-teachers-content.tsx:660, 715`; `schools-years-content.tsx:78, 90`. Done for all six files. The Title Case that remains is in the `MasterDataShell` titles the page files pass, which this list did not reach.
8. Native dates to the design-system picker (sheet on phone) across the 18 files.
9. Library loans: app-bar title "Library loans". Done.
...
12. Explanatory paragraphs: keep only those that carry a changing number, as a band chip. Done on the back-office boards, where a card carrying a number kept the number as a chip and lost the prose; the descriptions under the setup titles are untouched.
...
**Interaction patterns to add**: bulk actions (invite, link to HR, activate structures); ⌘K verbs ("New class", "Open the timetable for Form 3"); inline quick-edit for one-field changes (form teacher, capacity, pass mark); URL-synced filters; undo toasts for archive and delete; requirement banners as the only disabled state; route-level `loading.tsx`, `error.tsx`, `not-found.tsx`. The last of these has landed across `app/schools`: a skeleton, a client error boundary wired to its own reset and quoting the digest, and a not-found; where a route's shape is distinctive the skeleton mirrors it, so the timetable is a week grid and the record pages are the two-column property rail they actually draw, and the three class routes that call `notFound()` send the reader back to the list they came from. Nothing below the segment root carries its own boundary, deliberately. The six master-data pages under Management have none of this. Of the bulk actions, only a Remind shipped, on the fees ledger: bulk issue, print, activate and approve have no endpoint behind them and were deliberately not faked by fanning out requests from the browser, so bulk invite, bulk link to HR and activating structures in a batch wait on the same thing.

## 8. Accessibility

- Clipped actions on teachers are unreachable by keyboard; the menu fixes it. The menu has since landed, and the repeated per-row button with it.
