...
| `/schools/teachers` | `schools-teachers-content.tsx` (1,027 lines) | `PageHeading` plus caption duplicating the band; "Add Subject" Title Case; Title Case tabs; three control rows; "Find the employee" repeated on all eight rows (confirmed at runtime); the clipped Actions column seen in the older screenshot set was not reproduced at 1440px | Three tabs, three deletes; subjects CRUD here and in master data | gated | renderer ×3 (good) | P1 |
...
- **Teachers and teacher-HR.** Identity cell with mono id and email (good). Heading, caption and bar title; "Add Subject" in Title Case; three control rows; "Find the employee" eight times. The runtime capture shows the primary in the bar and no clipped column, so the older screenshot predates a fix.
