...
/** Catalogue items exist only to carry a `taxPercent` back to the sale line. */
...
type LineSpec = { catalogItemId: string; taxAmount: number; lineTotal: number };
...
async function makeSale(input: {
  ...
  return prisma.retailSale.create({
    data: {
      ...
      subtotal: input.totalAmount - input.taxAmount,
...
function standardSale() {
  return makeSale({
    totalAmount: 115,
    taxAmount: 15,
    lines: [{ catalogItemId: itemVat15, taxAmount: 15, lineTotal: 115 }],
  });
}
...
beforeAll(async () => {
  ...
  const catalogue = await Promise.all(
    [
      ...
      prisma.retailCatalogItem.create({
        data: {
          companyId,
          catalogCode: `${spec.code}-${suite}`,
          sku: `${spec.code}-${suite}`,
          inventoryItemId: `inv-${suite}-${spec.code}`,
          siteId: `site-${suite}`,
          name: spec.code,
          unitPrice: 100,
...
describe("fiscalising one till sale", () => {
  ...
  it("signs the exact minor units and nothing a float ever touched (invariant 5)", async () => {
    ...
    const sale = await makeSale({
      totalAmount: total,
      taxAmount: 3.95,
      lines: [{ catalogItemId: itemVat15, taxAmount: 3.95, lineTotal: total }],
    });
  ...
  it("refuses a sale whose rate has no taxID, and reserves no number for it (invariant 4)", async () => {
    ...
    const sale = await makeSale({
      totalAmount: 105,
      taxAmount: 5,
      lines: [{ catalogItemId: itemUnmapped, taxAmount: 5, lineTotal: 105 }],
    });
  ...
  it("refuses a sub-cent amount rather than rounding it into a signature (invariant 5)", async () => {
    ...
    const sale = await makeSale({
      totalAmount: 10.005,
      taxAmount: 1.3,
      lines: [{ catalogItemId: itemVat15, taxAmount: 1.3, lineTotal: 10.005 }],
    });
