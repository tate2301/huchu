 ...
 *
 * **1. Money is `Float` here, and a signature cannot be.** `RetailSale` and
 * `RetailSaleLine` hold doubles (`totalAmount`, `taxAmount`, `lineTotal`). The
 ...
 * has to happen somewhere, and it happens *here*, at the last point that still
 * knows which document the number came from. Doing it deeper (inside the
 * signer) would make "this is $10.30" and "this is a rounding artefact of
 * 10.299999999999999" indistinguishable; doing it shallower (in the sync route)
 * would put the boundary in a request handler where nobody looking for it would
 * think to check. `centsFromAccountingAmount` is reused rather than
 ...
 *
 * **3. The till carries a rate, not a tax code.** `RetailCatalogItem.taxPercent`
...
export type RetailSaleLineForSigning = {
  ...
  taxAmount: number;
  lineTotal: number;
  /** `RetailCatalogItem.taxPercent` as it stood when the line was resolved. */
  taxPercent: number;
};

export type RetailSaleForSigning = Pick<
  RetailSale,
  "saleNo" | "saleType" | "currency" | "totalAmount" | "taxAmount"
> & { receiptDate: Date };
...
export function buildRetailSaleSigningInput(input: {
  ...
  const receiptTotal = centsFromAccountingAmount(
    sale.totalAmount,
  ...
  for (const line of lines) {
    const mapping = resolver.resolve(line.taxPercent, `${line.itemName} on ${sale.saleNo}`);
    const taxCents = centsFromAccountingAmount(
      line.taxAmount,
      `tax on ${line.itemName} (${sale.saleNo})`,
    );
    const lineCents = centsFromAccountingAmount(
      line.lineTotal,
  ...
  const declaredTaxCents = centsFromAccountingAmount(
    sale.taxAmount,
...
/**
 * The rate for each line, taken from the catalogue item it was sold from.
 *
 * `RetailSaleLine` stores the tax *amount* but not the rate, so the rate has to
 * come back from `RetailCatalogItem.taxPercent`. A line whose catalogue item is
 * gone — or that never had one — is refused rather than defaulted to zero:
 * "zero-rated" is a VAT treatment, not a fallback, and a sale signed at 0%
...
async function resolveLineRates(
  ...
  const catalogItemIds = [
    ...new Set(sale.lines.map((line) => line.catalogItemId).filter((id): id is string => Boolean(id))),
  ];
  const catalogItems = catalogItemIds.length
    ? await prisma.retailCatalogItem.findMany({
        // No status filter: a delisted item's historical sale still has to
        // fiscalise, and the rate it was sold at is still on the row.
        where: { companyId, id: { in: catalogItemIds } },
        select: { id: true, taxPercent: true },
      })
    : [];
  const rateByCatalogItem = new Map(catalogItems.map((item) => [item.id, item.taxPercent]));

  return sale.lines.map((line) => {
    const taxPercent = line.catalogItemId ? rateByCatalogItem.get(line.catalogItemId) : undefined;
    if (taxPercent === undefined) {
      throw new RetailFiscalMappingError(
        "RETAIL_LINE_TAX_UNKNOWN",
        `${line.itemName} on ${sale.saleNo} has no catalogue item to take a tax rate from, so its ZIMRA taxID cannot be resolved`,
      );
    }
    return {
