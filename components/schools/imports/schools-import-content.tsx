...
export function SchoolsImportContent() {
  ...
  return (
    // A column of form, which is the one shape `narrow` is for: the wizard is
    // read and filled top to bottom, and a step laid across 1,600px of a wide
    // screen is a line of text nobody can follow back to its start.
    <SchoolsPage width="narrow">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <TableControls
              ...
              filterCount={historyFilter ? 1 : 0}
              count={`${history.length} of ${allHistory.length}`}
            />
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        {/* An affordance, not a field — but the head still needs
                            the cell, or every body row carries one more than the
                            head and the columns shift by one. */}
                        <TableHead>
                        ...
                        <TableRow key={record.id}>
                          ...
                          <TableCell className="text-right">
                            {record.status === "COMMITTED" ? (
                              <RecordActions
                                layout="menu"
                                label={`Row actions for ${record.fileName}`}
