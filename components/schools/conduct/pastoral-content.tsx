...
import { Lock, Policy } from "@/lib/icons";
import {
  askToSeeNote,
  fetchPastoralNotes,
 ...
 *
 * **The readers register comes first.** Before the notes, not after: the reader
 * is told who can see this before they are shown anything to see.
 *
...
export function PastoralContent() {
  ...
  return (
    ...
    >
      ...
      {/* Region 1, and it comes first on purpose. */}
      {readersOpen || readers ? (
        <section className="space-y-2">
          <h2 className="flex items-baseline justify-between border-b border-[color:var(--border-subtle)] pb-1.5">
            ...
            <span className="text-xs text-[color:var(--text-muted)]">
              {readers
                ? `${readers.readers.filter((reader) => reader.cleared).length} of ${readers.readers.length} cleared · ${readers.never.length} never`
                : ""}
            </span>
          </h2>
          {readersQuery.error ? (
      ...
      <section className="space-y-2">
        <h2 className="flex items-baseline justify-between border-b border-[color:var(--border-subtle)] pb-1.5">
          <span className="text-sm font-semibold text-[color:var(--text-strong)]">Notes</span>
          <span className="text-xs text-[color:var(--text-muted)]">
            {counts
              ? `${notes.filter((note) => note.readable).length} of ${counts.youMayRead} · ${counts.withheldFromYou} withheld`
              : ""}
          </span>
        </h2>
