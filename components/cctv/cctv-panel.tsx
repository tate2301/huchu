...
export function CCTVMetricStrip({ stats, className }: CCTVMetricStripProps) {
  return (
    ...
    >
      {stats.map((stat) => (
        <div key={String(stat.label)} className="bg-[var(--surface-base)] px-4 py-3">
          <div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">
            {stat.label}
          </div>
          <div className={cn("mt-1 text-2xl font-semibold tracking-tight", toneClass[stat.tone ?? "default"])}>
            {stat.value}
          </div>
          {stat.detail ? (
...
export function CCTVToolbar({ children, className }: CCTVToolbarProps) {
  return (
    <div
      className={cn(
        "flex flex-wrap items-center gap-2 rounded-2xl border border-[var(--edge-default)] bg-[var(--surface-base)] px-3 py-3",
        className,
      )}
    >
      {children}
