...
import { findUnsoldPours } from "@corelithzw/module-gold/gold/reconcile"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pours = await findUnsoldPours(prisma, { companyId, siteId, asOf })
    return successResponse(pours)
  } catch (error) {
    console.error("[API] GET /api/gold/reports/unsold-pours error:", error)
    return errorResponse("Failed to fetch unsold pours")
  }
}
