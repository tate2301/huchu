...
/**
 * S-4.3 — the school's records, as record pages.
 *
 * Driven by opening a real pupil from the list rather than by visiting a
 * hard-coded id, so this proves the route a registrar actually takes.
 *
 ...
 * it, so the phone run is testing different code from the desktop one.
 *
 * ## Why this is not `record-pages-suite.spec.ts`
 *
 * That suite's "School master-data records" describe covers
 * `/management/master-data/schools/classes/[id]` and `/subjects/[id]`. The five
 * `/schools/*` record pages — students, guardians, teachers, classes, subjects —
 * are a different set of routes, and so is everything this file asserts on them:
 * the "N more properties" disclosure and the custom field behind it, the
 * Guardians / Notes / Files / Enrolments / Teaches / Roll / Children tabs,
 * "Without a teacher" on the subject page, and reaching a subject by clicking
 * its code on the list rather than by typing its URL.
 *
 * Migrated off `chisipite-demo` and the `VISUAL_PASS=1` gate. Every id was
 * already resolved from the API, so nothing needed rewriting to move tenant —
 * except the hostel case, which now skips loudly rather than silently, because
 * `seed-school-demo.ts` writes no hostels.
 */
...
for (const viewport of [
  ...
  test.describe(`${viewport.name}`, () => {
    ...
    test("student record page", async ({ page }) => {
      const shot = shooter("schools", `records-${viewport.name}`);

      // Find a real pupil through the API rather than guessing an id, so the
      // spec keeps working when the tenant is reseeded. Prefer one with
      ...
      expect(candidates.length, "the school tenant has no students to open").toBeGreaterThan(0);
      ...
      if (await more.count()) {
        ...
        await shot(page, "student-custom-fields");
      }
      ...
      await shot(page, "student-guardians");
    ...
    test("guardian record page", async ({ page }) => {
      ...
      expect(candidates.length, "the school tenant has no guardians to open").toBeGreaterThan(0);
    ...
    test("teacher record page", async ({ page }) => {
      ...
      expect(candidates.length, "the school tenant has no teachers to open").toBeGreaterThan(0);
    ...
    test("subject record page", async ({ page }) => {
      ...
      const body = await list.json();
      const candidates: { id: string; code: string; name: string }[] = body?.data ?? [];
      expect(candidates.length, "the school tenant has no subjects to open").toBeGreaterThan(0);
      ...
      // existing — so the click is the thing worth proving.
      await expect(async () => {
    ...
    test("hostel record page", async ({ page }) => {
      ...
      const candidates: { id: string; code: string; name: string }[] = body?.data ?? [];
      /*
        `seed-school-demo.ts` writes thirteen entity types and a hostel is not
        one of them, so on St Mary's this skips rather than fails. It is kept
        because the assertions below are the only ones anywhere that open a
        hostel record, and they cost nothing until the seed grows a boarding
        house — at which point they start running on their own.
      */
      test.skip(candidates.length === 0, "the school seed writes no hostels");
